...
int exec_builtin(int argc, char** argv, int* fd) {
    ...
    if (strcmp(argv[0], "cd") == 0) {
        ...
        if (argc == 3) {  // use specified signal
            Kill(pid, atoi(argv[2]));
        } else {  // use default signal `SIGTERM`
            Kill(pid, SIGTERM);
        }
        return 0;
    } else {
...
int main() {
    ...
    char* commands[128];                // commands seperated by "|"
    char cwd[MAX_CWD_LENGTH];           // current working directory
    char* multi_commands[MAX_CMD_NUM];  // multi commands seperated by ";"

    int cmd_count;        // # of commands seperate by "|" in a single command
    int multi_cmd_count;  // # of commands seperated by ";"

    while (1) {
        /* TODO: 增加打印当前目录，格式类似"shell:/home/oslab ->"，你需要改下面的printf */
        // printf("shell: -> ");
        getcwd(cwd, MAX_CWD_LENGTH);  // get the current working directory
        printf("shell:%s ->", cwd);
        fflush(stdout);
        ...
        multi_cmd_count = split_string(cmdline, ";", multi_commands);  // split multi commands
        for (int cmd_i = 0; cmd_i < multi_cmd_count; cmd_i++) {
            /* 由管道操作符'|'分割的命令行各个部分，每个部分是一条命令 */
            /* 拆解命令行 */
            cmd_count = split_string(multi_commands[cmd_i], "|", commands);  // split pipe
            ...
            if (cmd_count == 0) {
                ...
                /* TODO:处理参数，分出命令名和参数 */
                argc = split_string(commands[0], " ", argv);  // seperated by " "
                ...
                for (int i = 0; i < cmd_count; i++) {
                    ...
                    if (pid == 0) {
                        ...
                        if (i < cmd_count - 1) {
                            ...
                            dup2(pipefd[WRITE_END], STDOUT_FILENO);  // redirect
                        ...
                        if (i > 0) {
                            ...
                            dup2(read_fd, STDIN_FILENO);  // redirect
                            close(read_fd);               
                        }
                    ...
                    close(pipefd[WRITE_END]);  // close parent's write end
